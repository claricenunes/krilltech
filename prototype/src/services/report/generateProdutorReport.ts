...
/**
 * Mesmo conteúdo do PDF (resumo, fatores, evolução do score, exposição x
 * capacidade, histórico de eventos), em CSV — abre direto no Excel.
 */
export function generateProdutorReportExcel({
  produtor,
  score,
  sintese,
  trendPoints,
  timeline,
}: ProdutorReportInput): void {
  const linhas: string[] = []

  linhas.push('Relatório de Risco de Crédito - KrillRadar (protótipo de demonstração)')
  linhas.push('')
  linhas.push(`Cliente;${csvEscape(produtor.nome)}`)
  linhas.push(`CNPJ;${csvEscape(formatCnpj(produtor.cliente_id))}`)
  linhas.push(`Região;${csvEscape(produtor.regiao)}`)
  linhas.push(`Cultura;${csvEscape(produtor.cultura)}`)
  linhas.push(`Score;${score.score}`)
  linhas.push(`Rating;${score.rating}`)
  linhas.push(`Classificação;${csvEscape(produtor.classificacao)}`)
  linhas.push(`Emitido em;${csvEscape(new Date().toLocaleDateString('pt-BR'))}`)
  linhas.push('')

  linhas.push('Fatores de risco')
  linhas.push('Fator;Peso (%);Status;Justificativa')
  for (const fator of score.fatores) {
    linhas.push(`${csvEscape(fator.title)};${fator.weight};${csvEscape(fator.status)};${csvEscape(fator.description)}`)
  }
  linhas.push('')

  if (sintese?.texto_explicativo) {
    linhas.push('Explicação do score')
    linhas.push(csvEscape(sintese.texto_explicativo))
    linhas.push('')
  }

  if (sintese?.recomendacao) {
    linhas.push('Recomendação do KrillRadar')
    linhas.push(csvEscape(sintese.recomendacao))
    linhas.push('')
  }

  if (trendPoints.length > 0) {
    linhas.push('Evolução do score: histórico, atual e projetado')
    linhas.push('Momento;Score;Rating;Observação')
    for (const point of trendPoints) {
      linhas.push(
        `${csvEscape(point.label)};${Math.round(point.score)};${csvEscape(point.rating)};${csvEscape(
          point.projected ? 'Cenário simulado para os próximos 12 meses — não é um fato.' : 'Realizado.',
        )}`,
      )
    }
    linhas.push('')
  ...
  const covered = produtor.margem_liquida >= produtor.exposicao
  linhas.push('Exposição x capacidade de pagamento')
  linhas.push('Item;Valor;Observação')
  linhas.push(`Exposição atual (KRILLTECH);${csvEscape(formatCurrency(produtor.exposicao))};`)
  linhas.push(
    `Margem líquida do produtor;${csvEscape(formatCurrency(produtor.margem_liquida))};${csvEscape(
      covered ? 'Cobre a exposição atual.' : 'Não cobre a exposição atual — sinal de atenção.',
    )}`,
  )
  linhas.push(`Limite de crédito aprovado;${csvEscape(formatCurrency(produtor.limite_credito))};`)
  linhas.push('')

  if (timeline.length > 0) {
    linhas.push('Histórico de eventos')
    linhas.push('Data;Tipo;Evento;Descrição')
    for (const evento of timeline) {
      linhas.push(`${csvEscape(evento.data)};${csvEscape(evento.tipo)};${csvEscape(evento.titulo)};${csvEscape(evento.descricao)}`)
    }
  }
  ...
  link.href = url
  link.download = `relatorio-risco-${slug || produtor.cliente_id}.csv`
